...
USING (
    SELECT
        ...
        COALESCE(s.retrans_orig_pkts, []) AS retrans_orig_pkts,
        COALESCE(s.retrans_resp_pkts, []) AS retrans_resp_pkts,
...
ON dest.uid = src.uid
WHEN MATCHED THEN UPDATE SET ingested_at = src.ingested_at
