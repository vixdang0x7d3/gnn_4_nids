"""
NetFlow-based GNN Training for UNSW-NB15 Intrusion Detection
Trains a Graph Convolutional Network on NetFlow features with MLflow tracking
"""
...
def _(mo):
    ...
    Configure model hyperparameters and paths for NetFlow-based GNN training.
    """)
    return


@app.cell
def _(duckdb, os, osp, project_root):
    conn = duckdb.connect()
    ...
    params = {
        # Paths
        "dataset_path": str(project_root / "model_training" / "data" / "nf_nb15_v3_preprocessed"),
    ...
    EXPERIMENT_NAME = "NetFlow_GNN_NIDS"
...
def _(
    ...
    # Clean data: replace inf with NaN, then fill with column max/min
    train_df = train_arrow.to_pandas()
    for col in FEATURE_ATTRIBUTES:
    ...
    return scaler_save_path, test_arrow, train_arrow, val_arrow
...
def _(
    ...
    with mlflow.start_run(run_name=f"run_{model_name}_{now.strftime('%Y%m%d%H%M%S')}") as run:
        ...
        if history:
            ...
            # Save and log figure
            history_plot_path = osp.join(artifacts_path, "training_history.png")
...
def _(
    ...
    if params["binary_mode"]:
        ...
        # Save and log
        curves_path = osp.join(artifacts_path, "roc_pr_curves.png")
        ...
        # Save and log
        cm_path = osp.join(artifacts_path, "confusion_matrix.png")
