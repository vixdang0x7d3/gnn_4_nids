...
# Configuration
CONTAINER_NAME="zeek-kafka-stream"
IMAGE="zeek_streaming:latest"
NETWORK="ml_platform"
KAFKA_BROKER="${KAFKA_BROKER:-broker:29092}"
KAFKA_TOPIC="${KAFKA_TOPIC:-zeek.logs}"
...
Examples:
    $0 live wlo1                           # Capture from wlo1
    $0 pcap ../test_pcaps/sample.pcap      # Process PCAP file
...
run_live() {
    ...
    echo "Implementation: Original (unsw-extra.zeek)"
    ...
    docker run -d \
        ...
        "$IMAGE" \
        zeek -C -i "$interface" \
            /scripts/unsw-extra.zeek \
            /scripts/writekafka.zeek
